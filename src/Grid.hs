...
-- | @emptyMineGrid@ will always snap the dimensions of the generated grid to
--   odd numbers so that there is always a center square at coordinate (0,0).
--   Even-numbered dimensions will automatically be increased by one to meet
--   this constraint.
...
-- | @randomMineGrid@ generates a random grid of mines using the supplied random
--   number generator. The second parameter is the probability of there being a
--   mine at any given location.
--   
--   * If the probability given is below zero, the generated grid will contain
--     no mines.
--   
--   * If the probability given is above one, the generated grid will be
--     completely mined.
...
-- | @gridBounds@ returns the minimum and maximum coordinates of the 'MineGrid'
--   in the form @((xMin, yMin), (xMax, yMax))@.
gridBounds ( MineGrid g ) = bounds g


-- | @gridSize@ returns the dimensions of the 'MineGrid' in the form
--   @(width, height)@
...
stateForLocation g l = if not $ inRange ( gridBounds g ) l
	then Nothing
...
minesNeighboringLocation g ( x , y ) = length mineNeighbors
	where
		neighbors = catMaybes $ map
			( \ l -> stateForLocation g l )
			( neighborCells g ( x , y ) )
		mineNeighbors = filter isMine neighbors

-- | @neighborsOfLocation@ is the list of @(location, 'LocationState')@ pairs
--   for all neighbors of a location
...
unclearedLeft g = length $ filter
	( \ loc -> ( not $ isCleared loc ) && ( not $ isMine loc ) )
	( elems $ arrayForGrid g )
...
alterLocation g l f = case stateForLocation g l of
	Nothing -> g
	Just s  -> if not $ inRange ( gridBounds g ) l
		then g
